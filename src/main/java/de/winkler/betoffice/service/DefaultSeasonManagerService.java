...
public class DefaultSeasonManagerService extends AbstractManagerService implements SeasonManagerService {
	...
	@Override
	@Transactional
	public Game addMatch(GameList round, ZonedDateTime date, Group group, Team homeTeam, Team guestTeam, int homeGoals,
			int guestGoals) {
	...
	@Override
	@Transactional
	public Game addMatch(Season season, int round, ZonedDateTime date, GroupType groupType, Team homeTeam,
			Team guestTeam) {

		Season persistedSeason = seasonDao.findById(season.getId());
		return (addMatch(persistedSeason.getGamesOfDay(round), date, persistedSeason.getGroup(groupType), homeTeam,
				guestTeam));
	}
	...
	@Transactional
	public Game addMatch(Season season, int round, ZonedDateTime date, GroupType groupType, Team homeTeam,
			Team guestTeam, int homeGoals, int guestGoals) {

		Season persistedSeason = seasonDao.findById(season.getId());
		return (addMatch(persistedSeason.getGamesOfDay(round), date, persistedSeason.getGroup(groupType), homeTeam,
				guestTeam, homeGoals, guestGoals));
	}
	...
	public Group addTeam(Season season, GroupType groupType, Team team) {
		...
		if (!season.getTeamType().equals(team.getTeamType())) {
			messages.add(new BetofficeValidationMessage("Der Meisterschaft unterstützt diesen Mannschaftstyp nicht.",
					null, Severity.ERROR));
		}
	...
	public Season createSeason(Season season) {
		try {
			...
			List<BetofficeValidationMessage> messages = new ArrayList<>();
			messages.add(
					new BetofficeValidationMessage("Diese Meisterschaft ist bereits vorhanden.", null, Severity.ERROR));
			throw new BetofficeValidationException(messages);
		}
	}
	...
	public void deleteSeason(Season season) {
		...
		// TODO Wenn die 'Season' Teil einer Community ist, dann kann diese nicht
		// entfernt wernde.

		if (findRounds(season).size() > 0) {
			messages.add(new BetofficeValidationMessage("Der Meisterschaft sind Spieltage zugeordnet.", null,
					Severity.ERROR));
		}

		if (findGroups(season).size() > 0) {
			messages.add(
					new BetofficeValidationMessage("Der Meisterschaft sind Gruppen zugordnet.", null, Severity.ERROR));
		}
	...
	public Season addGroupType(Season season, GroupType groupType) {
		...
		//
		// TODO Eine NoResultException fuehrt zu einem Transaktions-Rollback
		// (UnexpectedRollbackException).
		//
		// Spring ist fuer diese Funktion zustaendig: RuntimeExceptions fuehren zu einem
		// Rollback.
		//
		// Das Catch der 'NoResultException' kann die Rollback Exception nicht
		// verhindern.
		//
		// try {
		// Group group = groupDao.findBySeasonAndGroupType(season, groupType);
