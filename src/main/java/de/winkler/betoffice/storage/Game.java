...
public class Game extends AbstractStorageObject implements Comparable<Game> {
	 ...
	 * @see #setResult(GameResult)
	 */
	public void setResult(final int homeGoals, final int guestGoals, final boolean finished) {
	 ...
	 */
	public void addTipp(final GameTipp tipp) throws StorageObjectExistsException {
	 ...
	 */
	public void removeTipp(final GameTipp tipp) throws StorageObjectNotFoundException {
	 ...
	 */
	public GameTipp getGameTipp(final User user) throws StorageObjectNotFoundException {
		Validate.notNull(user, "user als null Parameter");

		Optional<GameTipp> userTipp = tippList.stream().filter(tipp -> tipp != null && tipp.getUser().equals(user))
				.findFirst();
		...
		if (userTipp.isPresent()) {
			...
			throw new StorageObjectNotFoundException(
					"Der Teilnehmer [" + user.getNickName() + "] hat keinen Spieltipp.");
		}
	}
